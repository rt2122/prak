...
unsigned
decode(char *buf, int *idx, int len_buf)
{
    unsigned char c;
...
main(void)
{
    ...
    int len_max = 0, prev_len = 0, len_buf;
    while ((len_buf = read(0, buf, M)) > 0) {
            ...
            if (buf[i] <= ' ') {
                if (prev_len) {
                    if (prev_len + i - idx > len_max) {
                        ...
                        max = calloc(len_max + prev_len, sizeof(char));
                        snprintf(max, len_max + prev_len, "%s%s", prev_word, buf + idx);
                        free(prev_word);
                        prev_len = 0;
                    }
                } else {
